...
public class FileBytesRequest extends Message {
    public FileBytesRequest(PeerServer server, String pathName, FileDescriptor fileDescriptor, long position) {
        super("BYTES:" + pathName + ":" + fileDescriptor + ":" + position);

        long bytesLeft = Math.min(fileDescriptor.fileSize - position, server.getMaximumLength());
