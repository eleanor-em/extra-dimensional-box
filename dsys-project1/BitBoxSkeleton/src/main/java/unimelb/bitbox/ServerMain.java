...
class MessageProcessingThread extends Thread {
	...
	public MessageProcessingThread(ServerMain server, String mode) {
		this.server = server;
...
public class ServerMain implements FileSystemObserver {
	...
	public ServerMain() throws NumberFormatException, IOException, NoSuchAlgorithmException {
		...
		processor = new MessageProcessingThread(this, mode);
	...
	void acceptConnectionsUDP(){
		...
		while (true) {
			try {
				...
				String hostPort = packet.getAddress() + ":" + packet.getPort();
				String hostAddr = hostPort.replace("/", "");
				System.out.println("accepting connections "+hostPort);
				String message = new String(packet.getData(), 0, packet.getLength());
//				Document doc = Document.parse(message);
//				String command = doc.getString("command");
				String name = formatName(names.poll());
//
//				if(command == Message.HANDSHAKE_RESPONSE){
//					//check if it's valid
//
//				}
				//if received handshake response.
				//receive handshake request
				AtomicBoolean newPeer = new AtomicBoolean(false);
				PeerConnection connectedPeer = peers.stream()
													  .filter(peer -> hostAddr.equals(peer.getHost() + ":" + peer.getPort()))
													  .findFirst()
													  .orElseGet(() -> {
														newPeer.set(true);
														return new PeerUDP(name, this, PeerConnection.State.WAIT_FOR_REQUEST, udpSocket, packet, "server");
													  });
				if (newPeer.get()) {
					peers.add(connectedPeer);
				}
				System.out.println("current peer list: "+getConnectedAddresses());
				//processor.messages.add(new ReceivedMessage(new String(packet.getData()), connectedPeer));
			} catch (IOException e){
		...
		for (Iterator<String> i = peerAddresses.iterator(); i.hasNext(); ) {
			...
			if (parts.length > 1) {
				...
				try {
					...
					//it should send handhsake request without creating the peer.
					...
					System.out.println("client side, current peer list: "+getConnectedAddresses());
...
//					log.warning("accepting connection timed out");
//				}
				catch (Exception e){
					log.warning("Connection to peer `" + addr + "` failed: " + e.getMessage());
				}
